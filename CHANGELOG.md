...
### Fixed
- 修复检查模式在触摸设备上因提前拦截触摸事件而无法完成元素选择的问题
- 补齐 `onPointerDown` / `pointer` 交互场景的 Demo 与 E2E 回归覆盖
- 保证同一选择手势不会在早期事件与后续 click 间重复触发复制流程

## [1.2.1] - 2026-03-13

### Fixed
- 修复检查模式下点击目标元素时未完全拦截同级 capture click 监听的问题，避免 React / 宿主业务点击链路继续执行
- 调整 E2E 用例以匹配当前 Demo 的图片复制与“全部复制”交互路径
